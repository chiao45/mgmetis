# -*- coding: utf-8 -*-
import numpy as np
from mgmetis.metis import part_graph_recursize
...
def test_c():
    _, part = part_graph_recursize(4, *create_graph("int32"))
    assert (
...
def test_c64():
    ...
    _, part = part_graph_recursize(4, *create_graph())
    assert (
        sum(np.where(part == x)[0].size for x in range(4)) == len(create_graph()[0]) - 1
    )
...
def test_fortran():
    ...
    xadj += 1
    adjs += 1
    _, part = part_graph_recursize(4, xadj, adjs)
